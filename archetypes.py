...
get_path_to_spectrum = lambda plate, mjd, fiberid : os.path.join(os.environ['HOME'], 'SDSS', 'v5_10_7', str(plate).zfill(4), "spec-"+str(plate).zfill(4)+"-"+str(mjd).zfill(5)+"-"+str(fiberid).zfill(4)+".fits" )
...
ok=(cat['Z_ZQ']>=3.)&(cat['Z']>zmin)&(cat['Z']<=zmax)&(cat['CLASS']=="GALAXY")&(cat['PLATE']==9242)
...
n.savetxt("allflux.txt", allflux)
n.savetxt("allivar.txt", allivar)
n.savetxt("masterwave.txt", masterwave)

print("interpolates on the common grid")
index_wave_all = n.searchsorted(masterwave, [2400., 5200.])
...
chi2_min = n.median(chi2) # 0.05 # the minimum distance, the only free paramter
