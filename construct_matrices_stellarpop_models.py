...
path_2_eboss21_cata = os.path.join(os.environ['OBS_REPO'], 'SDSS/ELG/eboss22/cats/eboss22.v5_10_7.latest.fits')
...
get_path_to_spectrum = lambda plate, mjd, fiberid : os.path.join(os.environ['HOME'], 'SDSS', 'v5_10_7', 'stellarpop', str(plate).zfill(4), "spFly-"+str(plate).zfill(4)+"-"+str(mjd).zfill(5)+"-"+str(fiberid).zfill(4)+".fits" )

def get_spec(path_to_spectrum, maskLambda=maskLambda):
	"""
	for a spectrum, returns plate, mjd, fiberid
	"""
	print(path_2_spectrum)
...
name = "stellarpop_zmin_"+str(zmin)+"_zmax_"+str(zmax)+"_zQmin_"+str(zQ_min)+"_Ngal_"+str(nGal)
print("run name:",name)
