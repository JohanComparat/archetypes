...
index_wave_all = n.searchsorted(masterwave, [wlmin_rf+500, wlmax_rf-500])
...
for i in n.arange(0,imax,1):
	...
	ax.set_xlim(wlmin_rf, wlmax_rf)
	#ax.set_ylim(-0.1, 3)
	ax.set_xticks(n.arange(wlmin_rf, wlmax_rf, 500))
...
ax.set_xticks(n.arange(wlmin_rf, wlmax_rf, 500))
fig.savefig(os.path.join(os.environ['OBS_REPO'], 'archetypes', "archetypes_"+name+"_SN_"+str(sn_min)+".png"))
#fig.savefig(os.path.join(os.environ['HOME'], 'wwwDir', 'sdss', 'elg', 'test.png'))
p.clf()

n.savetxt(os.path.join(os.environ['OBS_REPO'], 'archetypes', "archetypes_"+name+"_SN_"+str(sn_min)+".txt")   , n.vstack((masterwave, tmpmedian)))
